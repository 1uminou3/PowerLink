...
function initializeCharts() {
    ...
    console.log('Initializing solar charts for user:', user.uid);
    const voltageRef = db.ref(`users/${user.uid}/data/SolarPanel_(Voltage)`);
    const currentRef = db.ref(`users/${user.uid}/data/SolarPanel_(Current)`);
            ...
            if (snapshot.exists()) {
                ...
                snapshot.forEach((child) => {
                    ...
                    if (value && typeof value.timestamp === 'number' && typeof value.value === 'number') {
                        ...
                        console.warn('Invalid current reading:', value);
                    }
                });
                ...
                if (currentData.length > 0) {
                    ...
                    console.warn('No valid current readings found in the snapshot');
                }
            } else {
    ...
    if (!voltageChart) {
        voltageChart = new Chart(voltageChartContext, {
            ...
            options: {
                ...
                title: {
                    display: true,
                    text: 'Solar Panel Production (Voltage)',
                    fontSize: 16,
                    padding: 20
                }
            }
    ...
    if (!currentChart) {
        currentChart = new Chart(currentChartContext, {
            ...
            options: {
                ...
                title: {
                    display: true,
                    text: 'Solar Panel Production (Current)',
                    fontSize: 16,
                    padding: 20
                }
            }
...
function checkChartElements() {
    ...
    console.log('Solar chart element:', document.getElementById('solarChart'));
